 ...
 * @since 26.1.0
 */
class HTDeferredFluidTypeRegister(namespace: String) :
    HTDeferredRegister<FluidType>(NeoForgeRegistries.Keys.FLUID_TYPES, namespace) {
    /**
    ...
    ): HTDeferredFluidType<TYPE> = this.register(name) { _ -> properties.let(factory) }

    //    HTDeferredRegister    //

    override fun <I : FluidType> createHolder(
        registryKey: RegistryKey<FluidType>,
        key: Identifier
    ): HTDeferredFluidType<I> = HTDeferredFluidType(key)
