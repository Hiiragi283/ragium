...
fun buildVariantState(block: Block, action: MutableList<BlockStateVariant>.() -> Unit): BlockStateSupplier {
...
fun <T : Comparable<T>> buildWhen(property: Property<T>, value: T): When.PropertyCondition =
    When.create().set(property, value)

fun buildWhen(action: When.PropertyCondition.() -> Unit): When = When.create().apply(action)

//    BlockStateVariant    //

fun buildStateVariant(action: BlockStateVariant.() -> Unit): BlockStateVariant =
    BlockStateVariant.create().apply(action)
