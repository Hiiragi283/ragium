...
class HTInfusingRecipeImpl(val ingredient: Ingredient, val result: HTItemOutput, val cost: Float) : HTInfusingRecipe {
    companion object {
        @JvmField
    ...
    override fun matches(input: HTInfusingRecipeInput, level: Level): Boolean = ingredient.test(input.stack)

    override fun assemble(input: HTInfusingRecipeInput, registries: HolderLookup.Provider): ItemStack = getResultItem(registries)
