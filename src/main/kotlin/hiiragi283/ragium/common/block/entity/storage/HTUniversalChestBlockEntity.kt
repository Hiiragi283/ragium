...
class HTUniversalChestBlockEntity(pos: BlockPos, state: BlockState) :
    ExtendedBlockEntity(RagiumBlockEntityTypes.UNIVERSAL_CHEST, pos, state),
    HTHandlerProvider,
    HTItemHandler {
    ...
    override fun getItemHandler(direction: Direction?): IItemHandler? {
        val server: MinecraftServer = RagiumPlatform.INSTANCE.getCurrentServer() ?: return null
        return RagiumPlatform.INSTANCE.getUniversalBundle(server, this.color)
    }
    ...
    override fun getItemSlots(side: Direction?): List<HTItemSlot> = (getItemHandler(side) as? HTItemHandler)?.getItemSlots(side) ?: listOf()
}
