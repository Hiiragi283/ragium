...
class HTWaterCollectorBlockEntity(
    pos: BlockPos,
    state: BlockState,
) : BlockEntity(RagiumBlockEntityTypes.WATER_COLLECTOR, pos, state), SidedStorageBlockEntity {
    ...
    companion object {
        ...
        val TICKER: BlockEntityTicker<HTWaterCollectorBlockEntity> =
            BlockEntityTicker { world: World, pos: BlockPos, _: BlockState, blockEntity: HTWaterCollectorBlockEntity ->
                ...
                val sideWaters: Int = Direction.entries
                    .filterNot { it.axis == Direction.Axis.Y }
                    .map(pos::offset)
                    .map(world::getFluidState)
                    .count { it.isIn(FluidTags.WATER) }
                val amount: Long = FluidConstants.BUCKET * when (sideWaters) {
                    2 -> 1.0
                    3 -> 1.5
                    4 -> 2.0
                    else -> 0.0
    ...
    private val internalTank: SingleFluidStorage = object : SingleFluidStorage() {
