    ...
    override fun appendTooltip(
        ...
        stack.get(RagiumComponentTypes.DRUM)?.let { storage: SingleFluidStorage ->
            if (!storage.isResourceBlank) {
                tooltip.add(Text.literal("- Fluid: ${FluidVariantAttributes.getName(storage.resource).string}"))
                tooltip.add(Text.literal("- Amount: ${storage.getAmount()}"))
                tooltip.add(Text.literal("- Capacity: ${storage.capacity}"))
            }
        }
    }
