...
object HTFluidPumpCategory : DisplayCategory<HTFluidPumpCategory.FluidDisplay> {
    override fun getCategoryIdentifier(): CategoryIdentifier<out FluidDisplay> = RagiumREIClient.FLUID_PUMP

    override fun getTitle(): Text = Text.literal("Fluid Pump")

    override fun getIcon(): Renderer = EntryStacks.of(Items.BUCKET)

    override fun setupDisplay(display: FluidDisplay, bounds: Rectangle): List<Widget> = buildList {
    ...
    class FluidDisplay(private val biomeKey: RegistryKey<Biome>, val item: HTFluidCubeItem) : Display {
        ...
        override fun getCategoryIdentifier(): CategoryIdentifier<*> = RagiumREIClient.FLUID_PUMP
    }
}
